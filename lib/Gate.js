...
function holdCallbacks (gate, context) {
    ...
    if (entry.substituted) {
        var disconnectedErr = new errors.DisconnectedError('Node disconnected.', {remote: false})
...
Gate.prototype.receive = function (data) {
    ...
    if (cbs) {
        ...
        var disconnectedErr = new errors.DisconnectedError('Node disconnected.', {remote: true})
