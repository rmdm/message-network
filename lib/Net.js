...
function makeContext (as, to, topic, success, error, caller, nodes) {
    ...
    context.reply = success ? makeResponseCtxReplyFn(success, params) : noop
    context.refuse = error ? makeResponseCtxErrorFn(error, params) : noop
...
function makeResponseCtxReplyFn (callback, params) {
    return function (response, options) {
        ...
        if (!ctxNode || !dstNode) {
            if (options.error) {
                var noopContext = makeNoopContext(params.as, params.to, params.topic)
...
function makeNoopContext (as, to, topic) {
    var sender = {node: as.node || as}
    if (as.gate) { sender.gate = as.gate }
    var context = {
        sender: sender,
