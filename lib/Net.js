...
function send (handlers, params, options) {
    ...
    for (var i = 0; i < destinations.length; i++) {
        ...
        sendToDestination(handlers, destination, params, options)
    }
}

function sendToDestination (handlers, destination, params, options) {
    if (destination.gate && params.as.gate) {
    ...
    var srcGateName = params.as.gate
    var srcNodeName = params.as.node || params.as
    var dstNodeNames = destination.node || destination

    if (srcGateName) {
        var gate = handlers.gates[srcGateName]
        if (!gate) { return }
        sendNodes(gate[srcNodeName], null, dstNodeNames, params, options)
        sendNodes(gate['*'], null, dstNodeNames, params, options)
        return
    }

    var dstGateNames = destination.gate

    if (!dstGateNames) {
        var gate = handlers.nodes
        sendNodes(gate[srcNodeName], null, dstNodeNames, params, options)
        sendNodes(gate['*'], null, dstNodeNames, params, options)
        return
    }

    if (!Array.isArray(dstGateNames)) {
        dstGateNames = [dstGateNames]
    }

    for (var i = 0; i < dstGateNames.length; i++) {
        var dstGateName = dstGateNames[i]
        var gate = handlers.nodes
        sendNodes(gate[srcNodeName], dstGateName, dstNodeNames, params, options)
        sendNodes(gate['*'], dstGateName, dstNodeNames, params, options)
    }

}

function sendNodes (nodes, gateName, nodeNames, params, options) {
    if (!nodes) { return }
    ...
    for (var i = 0; i < nodeNames.length; i++) {
        var nodeName = nodeNames[i]
        sendNode(nodes, gateName, nodeName, params, options)
    }
}

function sendNode (nodes, gateName, nodeName, params, options) {
    var destinationName = gateName || nodeName

    if (destinationName !== '*') {
        sendDestination(nodes[destinationName], gateName, nodeName, params, options)
        return
    }
    ...
    if (gateName) {
        var gateNames = nodeNames.filter(function (nodeName) {
            return options.gates[nodeName]
        })

        for (var i = 0; i < gateNames.length; i++) {
            var gateName = gateNames[i]
            sendDestination(nodes[gateName], gateName, nodeName, params, options)
        }
    } else {
        nodeNames = nodeNames.filter(function (nodeName) {
            return !options.gates[nodeName]
        })

        for (var i = 0; i < nodeNames.length; i++) {
            var nodeName = nodeNames[i]
            sendDestination(nodes[nodeName], gateName, nodeName, params, options)
        }
    }

}

function sendDestination (topics, gateName, nodeName, params, options) {
    if (!topics) { return }
