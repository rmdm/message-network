...
function normalizeDestinations (destinations) {
    ...
    for (var destination of destinations) {
        ...
        if (!destination.gate) {
            ...
            starFullGates.push(destination)
...
function fireHandlers (handlers, gateName, nodeName, params, options) {
    ...
    for (var handler of handlers) {
        ...
        var context = makeContext(params.as, to, params.topic, params.success, params.error, handler, options.nodes, {
            timeout: params.options ? params.options.timeout : 0,
        })

        var ctxNode = options.nodes[gateName || nodeName]
        var dstNode = options.nodes[params.as.gate || params.as.node || params.as]

        if (!ctxNode || !dstNode) {
            if (params.error) {
                var noopContext = makeContext(to, params.as, params.topic, null, null)
                var disconnectedError = new errors.DisconnectedError('Node disconnected', {remote: !ctxNode})
                params.error(disconnectedError, noopContext)
            }
            return
        }
...
function makeContext (as, to, topic, success, error, caller, nodes, options) {
    ...
    context.refuse = makeResponseCtxErrorFn(error || noop, params)
    context.reply = makeResponseCtxReplyFn(success || noop, params)
...
function makeResponseCtxReplyFn (callback, params) {
    return function (response, options) {
        ...
        var dstNode = params.nodes[params.to.gate || params.to.node]

        if (!ctxNode || !dstNode) {
            if (options.error) {
                var noopContext = makeContext(params.as, params.to, params.topic, null, null)
                var disconnectedError = new errors.DisconnectedError('Node disconnected', {remote: !ctxNode})
        ...
        var responseContext = makeContext(params.to, params.as, params.topic, options.success || params.caller, options.error, callback, params.nodes, {
            timeout: options.options ? options.options.timeout : 0,
        })
    ...
    return function (error, options) {
        ...
        if (!(error instanceof errors.BaseError)) {
            if (error instanceof Error) {
                error = errors.deserialize(error)
            } else {
                error = new errors.BaseError('Refused.', error)
            }
        }
